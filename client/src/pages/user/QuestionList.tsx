...
import useQuestionList from "../../hooks/user/useQuestionList";
// redux
...
export default function QuestionList(): JSX.Element {
  ...
  const [[products, count]] = useQuestionList(page, rowsPerPage);

  //   const products:Question[] = [{
  //     title: "string",
  //     questioner: "string",
  //     respondent: "string",
  //     timeout: "string",
  ...
  const handleClick = (status: string, id: string) => {
    if (status === "聊天中" || status === "订单完成") {
      ...
      navigate("/user/dashboard/questions/" + id + "/answer");
    } else if (status === "待接单") {
      navigate("/user/dashboard/questions/" + id + "/accept");
    } else if (status === "已回答") {
      navigate("/user/dashboard/questions/" + id + "/detail");
    } else {
      return;
    }
  };
